...
def main(epochs=20, target_epsilon=2.0, target_delta=1e-5, l2_norm_clip=3.0, 
    ...
    ## Configuration A: Comparable Privacy (ε ≈ 3.0) (only for 20 epochs, if number of epochs changed, gaussian ε will be different.)
    gaussian_config = {
        'l2_norm_clip': 3.0,
        'noise_multiplier': 1.1305, # Will give ε = 2.9978 (approx) after 20 epochs.
        'delta': 1e-5,
        'batch_size': 1024,
    ...
    noise_mult_values = [2.4414, 1.4536, 1.1305, 0.8835, 0.6777, 0.5901, 0.5375] # For gaussian (Calculated by binary search and dp_accounting)
    ...
    for noise_type in noise_types_to_test:
        ...
        path = f"Results_Privacy-Utility_Curves//{noise_type}_dp_results.json"
        ...
        combined_path = f"Results_Privacy-Utility_Curves//combined_dp_results.json"
