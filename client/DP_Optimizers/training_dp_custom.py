...
@tf.function
def clip_gradients_by_global_norm(per_example_grads, l2_norm_clip):
    ...
    def clip_single_example(example_idx):
        ...
        clip_coeff = tf.minimum(1.0, l2_norm_clip / (global_norm + 1e-8))
...
# ========================= CORRECTED Laplace Manager =========================

class LaplaceDPManager:
    """CORRECTED: Laplace mechanism with proper epsilon budget management"""
    def __init__(self, epsilon_total, l2_norm_clip):
        self.epsilon_total = float(epsilon_total)
        self.l2_norm_clip = float(l2_norm_clip)  # Sensitivity
    ...
    def setup_budget(self, total_steps):
        ...
        print(f"Laplace: epsilon_per_step = {self.epsilon_per_step:.6f}")
    
    def add_noise_to_gradients(self, aggregated_gradients):
        """Add Laplace noise with CORRECT scaling"""
        if self.epsilon_used + self.epsilon_per_step > self.epsilon_total:
            raise ValueError(f"Epsilon budget exhausted!")
        
        noisy_gradients = []
        
        # CORRECT Laplace scale: sensitivity / epsilon_per_step
        laplace_scale = self.l2_norm_clip / self.epsilon_per_step
...
class GaussianDPManager:
    """CORRECTED: Gaussian mechanism with RDP accounting - NO epsilon_total needed"""
    def __init__(self, noise_multiplier, l2_norm_clip, delta=1e-5):
        ...
        print(f"Gaussian: noise_scale = {self.noise_scale:.6f}")
    ...
    def add_noise_to_gradients(self, aggregated_gradients):
        """Add Gaussian noise with proper RDP tracking"""
        noisy_gradients = []
        ...
        for grad in aggregated_gradients:
            ...
            gaussian_noise = tf.random.normal(
                tf.shape(grad),
                mean=0.0,
                stddev=self.noise_scale,  # CORRECT: l2_norm_clip * noise_multiplier
                dtype=grad.dtype
            )
    ...
    def get_privacy_spent(self):
        ...
        if not self.rdp_accountant:
            return {'epsilon': 0.0, 'delta': self.delta, 'steps': self.step_count}
        
        try:
            epsilon = self.rdp_accountant.get_epsilon(target_delta=self.delta)
            return {'epsilon': float(epsilon or 0.0), 'delta': self.delta, 'steps': self.step_count}
        except Exception as e:
            return {'epsilon': 0.0, 'delta': self.delta, 'steps': self.step_count}

# ========================= CORRECTED Optimizers =========================

class LaplaceDPOptimizer(tf.keras.optimizers.Adam):
    """Laplace DP optimizer with epsilon budget"""
    def __init__(self, l2_norm_clip, epsilon_total, learning_rate=0.001, **kwargs):
    ...
    def setup_dp(self, model, total_steps):
        """Setup DP manager"""
        self.model = model
        self.dp_manager = LaplaceDPManager(
    ...
    def train_step_with_dp(self, x_batch, y_batch):
        """Training step with Laplace DP"""
        per_example_grads = compute_per_example_gradients(self.model, x_batch, y_batch)
        clipped_grads = clip_gradients_by_global_norm(per_example_grads, self.l2_norm_clip)
...
class GaussianDPOptimizer(tf.keras.optimizers.Adam):
    """Gaussian DP optimizer with RDP accounting"""
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        try:
            if self.noise_type == "gaussian":
                ...
                if hasattr(self.trainer_instance, 'laplace_optimizer'):
                    privacy_spent = self.trainer_instance.laplace_optimizer.get_privacy_spent()
                    epsilon = privacy_spent.get('epsilon', 0.0)
        except Exception as e:
...
class IoTModelTrainer:
    ...
    def train_model(self, X_train, y_train_cat, X_val, y_val_cat,
                   ...
                   # Laplace parameters
                   epsilon_total=400.0,  # Only for Laplace
                   # Gaussian parameters  
        ...
        elif use_dp and noise_type == 'laplace':
            return self._train_laplace_dp(
                X_train, y_train_cat, X_val, y_val_cat,
                epochs, batch_size, verbose, callbacks,
                l2_norm_clip, epsilon_total, learning_rate
            )
        else:
            # Non-DP training
    ...
    def _train_laplace_dp(self, X_train, y_train_cat, X_val, y_val_cat,
                         epochs, batch_size, verbose, callbacks,
                         l2_norm_clip, epsilon_total, learning_rate):
        """CORRECTED: Laplace DP training"""
        print(f"\n=== LAPLACE DP TRAINING ===")
        print(f"Parameters: epsilon_total={epsilon_total}, l2_norm_clip={l2_norm_clip}")
        
        steps_per_epoch = len(X_train) // batch_size
        total_steps = steps_per_epoch * epochs
        
        # Create Laplace optimizer
        self.laplace_optimizer = LaplaceDPOptimizer(
            l2_norm_clip=l2_norm_clip,
        ...
        # Setup DP budget
        self.laplace_optimizer.setup_dp(self.model, total_steps)
        ...
        return self._run_training_loop(
            X_train, y_train_cat, X_val, y_val_cat,
            epochs, batch_size, verbose, callbacks, 'laplace'
        )
